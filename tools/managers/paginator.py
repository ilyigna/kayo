...
from discord import Embed, HTTPException, Member, Message, Reaction, TextChannel, User
...
class Paginator(object):
    def __init__(self: "Paginator", ctx: "Context", data: List[Embed | str]):
        self.data: List[Embed | str] = data
        self.index: int = 0
        self.bot: "Kayo" = ctx.bot
        self.author: Member = ctx.author
        self.channel: TextChannel = ctx.channel  # type: ignore
    ...
    async def begin(self: "Paginator") -> Message:
        ...
        while self.message:
            ...
            elif self.reactions["skip"] == reaction:
                embed = Embed(
                    color=0x2B2D31,
